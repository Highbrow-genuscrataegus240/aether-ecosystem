          ...
          <div className="bg-[#0a0a0a] border border-white/5 rounded-2xl p-2 space-y-1">
            {pendingTasks.map((task) => (
              <div key={task.id} className="p-4 rounded-xl hover:bg-white/[0.03] transition-colors group flex gap-4 items-center">
                <button onClick={() => onToggleTask(task.id)} className="w-6 h-6 rounded-full border border-white/20 flex items-center justify-center text-transparent hover:border-violet-400 hover:text-violet-400 transition-colors shrink-0">
          ...
          <section>
            ...
            <div className="bg-[#0a0a0a] border border-white/5 rounded-2xl p-2 space-y-1 opacity-60">
              {completedTasks.map((task) => (
                <div key={task.id} className="p-4 rounded-xl hover:bg-white/[0.03] transition-colors group flex gap-4 items-center">
                  <button onClick={() => onToggleTask(task.id)} className="w-6 h-6 rounded-full bg-violet-500 border-violet-500 flex items-center justify-center text-white transition-colors shrink-0">
                    <Check className="w-3 h-3" />
                  </button>
                  <div className="flex-1 min-w-0">
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Add New Task">
        <form onSubmit={handleAddTask} className="space-y-4">
          <div>
            ...
            <input required type="text" value={title} onChange={e => setTitle(e.target.value)} className="w-full bg-black border border-white/10 rounded-lg px-4 py-2 text-white focus:outline-none focus:border-violet-500" placeholder="Follow up with client" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-400 mb-1">Due Time</label>
            <input required type="text" value={time} onChange={e => setTime(e.target.value)} className="w-full bg-black border border-white/10 rounded-lg px-4 py-2 text-white focus:outline-none focus:border-violet-500" placeholder="Tomorrow, 2:00 PM" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-400 mb-1">Type</label>
            <select value={type} onChange={e => setType(e.target.value as TaskType)} className="w-full bg-black border border-white/10 rounded-lg px-4 py-2 text-white focus:outline-none focus:border-violet-500 appearance-none">
          ...
          <div className="pt-4 flex justify-end gap-3">
            ...
            <button type="submit" className="bg-violet-500 hover:bg-violet-400 text-black px-4 py-2 rounded-lg text-sm font-medium transition-colors">Add Task</button>
          </div>
        </form>
      </Modal>
    </motion.div>
