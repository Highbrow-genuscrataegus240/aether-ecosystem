    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="max-w-7xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <div key={idx} className="bg-[#0a0a0a] border border-white/5 rounded-2xl p-6 hover:border-violet-400/20 transition-colors group">
            <div className="text-gray-400 text-sm font-medium mb-2 group-hover:text-violet-200 transition-colors">{stat.label}</div>
            <div className="flex items-end justify-between">
              <div className="text-3xl font-light text-white tracking-tight">{stat.value}</div>
              <div className={`text-xs font-medium px-2 py-1 rounded-full ${stat.trend.startsWith('+') ? 'bg-emerald-500/10 text-emerald-400' : 'bg-rose-500/10 text-rose-400'
                }`}>
                {stat.trend}
              </div>
            </div>
      ...
      <section>
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-medium text-violet-50">Sales Pipeline</h2>
          <button onClick={() => onNavigate('Pipeline')} className="text-sm text-violet-300 hover:text-violet-200 flex items-center gap-1 transition-colors">
            ...
            return (
              <div key={stage.id} className="bg-[#0a0a0a] border border-white/5 rounded-2xl p-4 flex flex-col h-[320px]">
                ...
                <div className="flex-1 overflow-y-auto space-y-3 pr-1 custom-scrollbar">
                  {stageDeals.map((item) => (
                    <div key={item.id} className="bg-black border border-white/10 rounded-xl p-4 hover:border-violet-400/30 transition-colors group">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <section className="lg:col-span-2">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-medium text-violet-50">Recent Clients</h2>
            <button onClick={() => onNavigate('Clients')} className="text-sm text-violet-300 hover:text-violet-200 flex items-center gap-1 transition-colors">
                  ...
                  <tr key={client.id} className="hover:bg-white/[0.02] transition-colors group">
                    <td className="px-6 py-4">
                      <div className="flex flex-col">
                        <span className="font-medium text-violet-50 group-hover:text-violet-200 transition-colors">{client.name}</span>
                        <span className="text-xs text-gray-500">{client.email}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-gray-400">{client.company}</td>
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium border ${client.status === 'Active' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' :
                    ...
                    <td className="px-6 py-4 text-right">
                      <button className="text-gray-500 hover:text-violet-300 transition-colors">
        ...
        <section>
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-medium text-violet-50">Upcoming Tasks</h2>
            <button onClick={() => onNavigate('Tasks')} className="w-8 h-8 rounded-full bg-white/5 hover:bg-white/10 flex items-center justify-center text-violet-300 transition-colors">
            ...
            {tasks.filter(t => !t.completed).slice(0, 5).map((task) => (
              <div key={task.id} className="p-4 rounded-xl hover:bg-white/[0.03] transition-colors group flex gap-4 items-start cursor-pointer" onClick={() => onToggleTask(task.id)}>
                <div className={`mt-1 w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${task.type === 'call' ? 'bg-blue-500/10 text-blue-400' :
                    task.type === 'email' ? 'bg-violet-500/10 text-violet-400' :
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-violet-50 group-hover:text-violet-200 transition-colors truncate">
                    {task.title}
                  </p>
                ...
                <button className={`w-5 h-5 rounded border flex items-center justify-center transition-colors mt-1 ${task.completed ? 'bg-violet-500 border-violet-500 text-white' : 'border-white/20 text-transparent hover:border-violet-400 hover:text-violet-400'
                  }`}>
